...
const AddMeeting = (props) => {
    const { onClose, isOpen, setAction, from, fetchData, view,meetingData ,setSelectedMeeting} = props
...
useEffect(() => {
    if (meetingData) {
        setValues({
            ...
            related: meetingData.related || 'None',
            dateTime: meetingData.dateTime || '',
            notes: meetingData.notes || '',
            createBy: user?._id,
        });
    }
}, [meetingData, setValues]);
    ...
    const saveMeeting = async () => {
        try {
            ...
            if (meetingData) {
                response = await putApi(`api/meeting/edit/${meetingData._id}`, values);
            } else {
                response = await postApi("api/meeting/add", values);
            }
    
            if (response && (response.status === 201 || response.status === 200)) {
                toast.success(meetingData ? "Meeting updated successfully!" : "Meeting added successfully!");
                onClose();
    ...
    return (
        ...
        onClose={() => {
            ...
            <ModalHeader>{meetingData ? "Edit Meeting" : "Add Meeting"}</ModalHeader>
                <ModalCloseButton />
